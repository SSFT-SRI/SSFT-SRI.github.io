...
(defun ssft-schedule-slots (day-start day-end slot-minutes events)
  (let ((seen (make-hash-table :test 'equal))
        slots)
    (cl-labels ((add-slot
                 (slot)
                 (when (and (<= day-start slot)
                            (< slot day-end)
                            (not (gethash slot seen)))
                   (puthash slot t seen)
                   (push slot slots))))
      (let ((slot day-start))
        (while (< slot day-end)
      ...
      (dolist (event events)
        (add-slot (ssft-event-start event))
        (add-slot (ssft-event-end event))))
    (sort slots #'<)))

(defun ssft-event-rowspan (start end slots)
  (let ((rowspan 0))
    (dolist (slot slots)
      (when (and (<= start slot)
                 (< slot end))
...
(defun ssft-index-events (events slots)
  (let ((starts (make-hash-table :test 'equal))
    ...
    (dolist (event events)
      (push event (gethash (list (ssft-event-date event)
                           ...
                           starts))
      (dolist (slot slots)
        (when (and (< (ssft-event-start event) slot)
                   (< slot (ssft-event-end event)))
...
(defun ssft-render-event-cell (events slots)
  (let* ((events (sort (copy-sequence events)
         ...
         (rowspan (ssft-event-rowspan start end slots))
...
(defun ssft-render-schedule-html ()
  (let* ((start-date (ssft-keyword "SSFT_SCHEDULE_START_DATE" "2026-05-23"))
         ...
         (slots (ssft-schedule-slots day-start day-end slot-minutes events))
         (indexes (ssft-index-events events slots))
         (starts (car indexes))
    ...
    (dolist (slot slots)
      ...
      (dolist (date dates)
        (let ((events-at-slot (gethash (list date slot) starts))
          ...
          (cond
           (events-at-slot
            (push (ssft-render-event-cell events-at-slot slots) rows))
           (is-covered
            nil)
           (t
